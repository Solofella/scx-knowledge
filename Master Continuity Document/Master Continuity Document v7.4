...
HOW Doc: agents/ALA/SCX_ALA_HOW_v4.md
Changelog: agents/ALA/SCX_ALA_CHANGELOG.md
...
Model: GPT gpt-5.2
Status: ✓ Verified operational
HOW Doc: agents/EIP/SCX_EIP_HOW_v4.md
...
Status: ✓ Verified operational
HOW Doc: agents/ESS/SCX_ESS_HOW_v4.md
...
Model: Claude claude-sonnet-4-6
Status: ✓ Verified operational
HOW Doc: agents/HSI/SCX_HSI_HOW_v3.md
...
Status: ✓ Verified operational
HOW Doc: agents/SIA/SCX_SIA_HOW_v3.md
...
Status: ✓ Verified operational
HOW Doc: agents/BRA/SCX_BRA_HOW_v3.2.md
...
Model: Claude claude-sonnet-4-6
Status: ✓ Verified operational
HOW Doc: agents/RDA/SCX_RDA_HOW_v3.1.md
Changelog: agents/RDA/SCX_RDA_CHANGELOG.md
...
Schema Registry v2
Location: schemas/Schema_Registry_v2.md
...
Protocol: protocols/SCX_PreBuild_Protocol_v1.0.md
...
Header (client name, period toggle)
Signal Pulse (T-NEG/T-AMB/T-POS distribution)
Signal Distribution (domain breakdown)
Response Draft Status (approved/pending counts)
Trend Signals (week-over-week changes)
SEO Signal (keyword hits, top keyword, coverage rate, response rate, avg velocity)
Period Toggle: This Week / Last Month

What's NOT shown: Review text, guest names, draft text, operational recommendations

Freelancer Brief: commercial/Dashboard_Freelancer_Brief.md
...
Unified flow: Platform adapters → Normalize to Google Sheet (7 cols: Date/Platform/Star Rating/Review Text/Reviewer Handle/Client ID/Ingestion Status) → n8n ALA reads from sheet daily → Dedup at sheet level (Reviewer Handle+Date+Platform)
...
xc-token (NocoDB Header Auth)
Subtext-CX-Anthropic (Header Auth with manual headers per node: anthropic-version:2023-06-01 + Content-Type:application/json)
Subtext-CX-OpenAI (Header Auth: Authorization: Bearer [key])
...
BCA Batch Controller - Option B design documented, build deferred pending volume analysis
RDA Internal Brief Quality - Must describe signal meaning only, no operational prescriptions
RDA Brand Voice Variation - Reduce from 3 phrases to 1 per draft, add variation constraint
Parent Brand Trading Name - Naming session deferred (candidates: Covero, Hostiq, Serviq, etc.)
subtextcx.com Landing Page - Must be live before outbound prospect contact
MRA Build - On hold until pilot data exists (5 pre-build items: table creation, RDA record count, email credential via Brevo, client display name, prior-month baseline)
...
Usage in Chats: Fetch from GitHub scx-knowledge:

agents/RDA/SCX_RDA_HOW_v3.1.md agents/RDA/SCX_RDA_CHANGELOG.md MCD_v7.4.md
